  ...
  const postular = async () => {
    ...
    try {
      ...
      if (!rutConductor || !patenteFurgon) {
        Alert.alert('Error', 'No se pudo obtener la informacion del furgón.');
        return;
      }

      const postulacionesRef = collection(db, 'postulacion_furgon');
      await addDoc(postulacionesRef, {
        rutPadre: rutUsuario,
        rutConductor,
      ...
      Alert.alert('Éxito', 'Se envio la solicitud al conductor.');
      router.back();
    } catch (error) {
      console.error('Error al postular:', error);
      Alert.alert('Error', 'No se pudo envio la solicitud.');
    }
  };
